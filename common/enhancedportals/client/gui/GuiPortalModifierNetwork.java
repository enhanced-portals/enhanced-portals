...
public class GuiPortalModifierNetwork extends GuiScreen
{
    ...
    public GuiPortalModifierNetwork(TileEntityPortalModifier modifier)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            elementList.add(new GuiGlyphElement(guiLeft + 8 + (i * 18), guiTop + 3, Reference.glyphValues.get(i), Reference.glyphItems.get(i), this));
        }
        
        for (int i = 9; i < 18; i++)
        {
            elementList.add(new GuiGlyphElement(guiLeft + 8 + ((i - 9) * 18), guiTop + 21, Reference.glyphValues.get(i), Reference.glyphItems.get(i), this));
        }
        
        for (int i = 18; i < 27; i++)
        {
            elementList.add(new GuiGlyphElement(guiLeft + 8 + ((i - 18) * 18), guiTop + 39, Reference.glyphValues.get(i), Reference.glyphItems.get(i), this));
        }
    ...
    public void initGui()
    {
        ...
        buttonList.add(new GuiButton(1, guiLeft + 7, guiTop + 95, 50, 20, "Cancel"));
        buttonList.add(new GuiButton(2, guiLeft + 63, guiTop + 95, 50, 20, "Random"));
        buttonList.add(new GuiButton(3, guiLeft + 119, guiTop + 95, 50, 20, "Accept"));
    }
    ...
    public void drawScreen(int x, int y, float par3)
    {
        ...
        int y2 = (height - ySize) / 2 - 15;
        drawTexturedModalRect(x2, y2, 0, 0, 166 + 22, 176 + 4);
        
        super.drawScreen(x, y, par3);
        
        GL11.glDisable(GL11.GL_LIGHTING);
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        fontRenderer.drawString("Glyphs", guiLeft + 7, guiTop - 9, 0xFF444444);
        fontRenderer.drawString("Network", guiLeft + 7, guiTop + 59, 0xFF444444);
        GL11.glEnable(GL11.GL_LIGHTING);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
        
        for (int i = stackList.size() - 1; i >= 0; i--)
        {
            stackList.get(i).drawElement(guiLeft + 8 + (i * 18), guiTop + 71, x, y, fontRenderer, itemRenderer, mc.renderEngine);
        }
        ...
        if (isShiftKeyDown())
        {
            ...
            GL11.glDisable(GL11.GL_LIGHTING);
            GL11.glDisable(GL11.GL_DEPTH_TEST);
            fontRenderer.drawString(EnumChatFormatting.GRAY + str, ((guiLeft + xSize / 2) - (fontRenderer.getStringWidth(str)) / 2), guiTop + ySize - 35, 0xFFFFFFFF);
            GL11.glEnable(GL11.GL_LIGHTING);
            GL11.glEnable(GL11.GL_DEPTH_TEST);
    ...
    protected void mouseClicked(int x, int y, int buttonClicked)
    {
        ...
        for (int i = 0; i < stackList.size(); i++)
        {
            stackList.get(i).handleMouseClick(guiLeft + 8 + (i * 18), guiTop + 71, x, y, buttonClicked);
        }
    }
