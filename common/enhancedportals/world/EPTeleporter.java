...
{
    public EPTeleporter(WorldServer par1WorldServer)
    {
            ...
            for (k2 = -1; k2 <= 1; ++k2)
            {
                for (i3 = 1; i3 < 3; ++i3)
                {
                    for (l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        worldServerInstance.setBlock(k3, j3, i4, flag ? Block.obsidian.blockID : 0);
                    }
                }
            }
        }
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        worldServerInstance.setBlock(k1, l1, i2, flag ? Block.obsidian.blockID : 0);
                    }
                }
            }
