...
public class PortalUtils
{
    ...
    public static ArrayList<TileEntity> getAllPortalComponents(TileFrameController controller) throws Exception
    {
        ArrayList<ChunkCoordinates> processed = new ArrayList<ChunkCoordinates>();
        ArrayList<TileEntity> portalComponents = new ArrayList<TileEntity>();
        ...
        while (!toProcess.isEmpty())
        {
            ...
            if (!processed.contains(c))
            {
                ...
                if (portalBlocks.contains(c) || t instanceof IPortalPart)
                {
    ...
    static Queue<ChunkCoordinates> getGhostedPortalBlocks(TileFrameController controller)
    {
        for (int j = 0; j < 6; j++)
        {
            for (int i = 1; i < 6; i++)
